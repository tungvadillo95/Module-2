...
namespace Search_Sort
{
    class Program
    {
        ...
        static void Find(int[] arr, int value)
        {
            ...
            {
                List<int> indexSearch = LinearSearch(arr, value);
                PrintArray(arr);
                if (indexSearch.Count != 0)
                {
                    Console.Write("Result search... Index value in array: ");
                    for (int i = 0; i < indexSearch.Count; i++)
                    {
                        Console.Write($"{indexSearch[i]}, ");
                    }
                    Console.WriteLine();
                }
                else
        ...
        static List<int> LinearSearch(int[] arr, int value)
        {
            List<int> indexSearch = new List<int>(0);
            for (int i = 0; i < arr.Length; i++)
            {
                if (arr[i] == value)
                {
